...
const Navbar = () => {
  ...
  useEffect(() => {
    // Simulate fetching cart and wishlist data from an API or state management
    setCartItemCount(user.cart.reduce((total, item) => total + item.qty, 0));
    setWishlistItemCount(user.wishlist.length);
  }, []);
  return (
    <>
      <div className="w-full md:px-40 bg-white h-16 shadow-md md:h-12 lg:h-14 mx-auto flex justify-between items-center text-black font-secondary">
        ...
        <div className="hidden md:flex w-[45%] h-full">
          <div className="w-full h-full flex items-center justify-center md:gap-x-5 px-2 tracking-wider font-bold  md:text-[16px] ">
            ...
            <Link to="/shop" className="px-1 border-b-2 border-white hover:border-b-2 hover:border-black transition-all duration-400">
              Shop
            </Link>
            <Link to="/" className="px-1 border-b-2 border-white hover:border-b-2 hover:border-black transition-all duration-400">
              About
        ...
        <div className=" w-[50%] md:w-[15%] h-full flex items-center justify-end pr-5 md:pr-2">
          <div className="flex gap-x-3 md:gap-x-4 items-center">
            <Link to="/wishlist" className="relative">
              ...
              <div className="absolute -top-2 -right-3 p-0.5 bg-red-500 text-white text-xs font-bold rounded-full flex items-center justify-center">{wishlistItemCount}</div>
            </Link>
            <Link to="/cart" className="relative">
              <ShoppingCart className="w-6 h-6" />
              <div className="absolute -top-2 -right-3 p-0.5 bg-red-500 text-white text-xs font-bold rounded-full flex items-center justify-center">{cartItemCount}</div>
            </Link>
            <Link to="/profile">
